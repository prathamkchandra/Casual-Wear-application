    ...
    <div className="space-y-5 sm:space-y-6">
      <section className="rounded-2xl bg-white shadow-soft p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-ink/50">Search Products</p>
          {query && (
            <button
              type="button"
              onClick={() => setQuery("")}
        ...
        <div className="mt-2">
          <input
            ...
            className="w-full rounded-full border border-ink/15 px-4 py-3 text-base font-medium focus:border-accent focus:outline-none"
          />
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 gap-5 lg:gap-8 lg:grid-cols-[240px_1fr]">
        <aside className="rounded-2xl bg-white shadow-soft p-4 space-y-4 h-fit lg:sticky lg:top-24">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.18em] text-ink/50">Category</p>
          ...
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.18em] text-ink/50">Price</p>
            <div className="mt-2 space-y-2">
        ...
        <div className="space-y-4">
          <p className="text-base font-semibold text-ink/60">{filtered.length} products</p>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((product) => (
              <ProductCard key={product._id.toString()} product={product} />
            ))}
            {filtered.length === 0 && (
              <div className="col-span-full text-center text-base font-medium text-ink/60">Nothing matches these filters.</div>
            )}
          </div>
        </div>
      </div>
...
function FilterButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full rounded-full border px-3 py-2 text-left text-base font-semibold transition-colors ${
        active
